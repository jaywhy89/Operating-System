...
main(int argc, char **argv)
{
	...
	if (setcontext_called == 1) {
		...
		/* Answer: No, since nothing changes setcontext_called till this line. */
		show_interrupt();
		exit(0);
	}
	...
	/* Answer: argc and argv are stored in rdi, rsi respectively. 
		       argv value is stored at uc_mcontext.gregs[9]. argc is not saved
		       However, rdi and rsi are caller saved */
	...
	/* Answer: Yes, they are stored on the stack, which grows down.
			   [10] = RBP,   [15] = RSP
	...
	*/
	...
	printf("number of bytes between err and the saved stack in mycontext "
	       "= %ld\n", (unsigned long)((void*)(&err)-mycontext.uc_mcontext.gregs[REG_RSP]));
	...
	/* Answer: uc_stack is the stack used by this context, but manipulated by makecontext.
	*/
	...
	/* Answer: setcontext does not return on successful call!
	*/
	assert(0);
}
